...
def bag_model_cv(X, y, nn_model):
    ...
    for train_index, test_index in kf.split(X):
        ...
        for j in range(nbags):
            ...
            nn_model.fit(X_model, y_model, batch_size=128, nb_epoch=nepochs, shuffle=True)
...
model = Sequential()
model.add(Dense(100, input_dim=data_train.shape[1], init='glorot_uniform'))
model.add(PReLU())
...
model = Sequential()
model.add(Dense(64, input_dim=data_train.shape[1], init='glorot_uniform'))
